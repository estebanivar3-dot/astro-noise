/**
 * Fill — flood-fill recoloring with random colors.
 * Full image mode recolors all distinct regions.
 * Interactive mode: paint areas to recolor them.
 */
...
const fillEffect: PixelEffect = {
  ...
  apply(source: ImageData, config: EffectConfig): ImageData {
    ...
    // Seeded PRNG for deterministic random colors
    let seed = Math.round(hueShift * 997 + tolerance * 13 + satShift * 71 + lumShift * 37);
    ...
    for (let i = 0; i <= maxLabel; i++) {
      regionHueOffsets[i] = rand() * hueShift;
    }
